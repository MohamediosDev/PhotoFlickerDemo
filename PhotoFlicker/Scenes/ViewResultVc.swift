...
class ViewResultVc: UIViewController {
    ...
    var pageCount = 1
    var searching = false
    let DataSearch:[String] = ["newyork","car","cairo"]
    ...
    func getPhoto(tag:String? = nil , page:String? = nil) {
        let url = "https://www.flickr.com/services/rest/?method=flickr.photos.search&api_key=c7c721d8a8bec200aaa94ebeb3917225&tags=\(tag ?? "")&format=json&extras=url_m&nojsoncallback=1&per_page=16&page=\(page ?? "")&pages=20"
        ApiService.Shared.fetchData(url: url, parms: nil, headers: nil, method: .get) {[weak self] (getData:PhotosModal?, failData:PhotosModal?, error) in
            ...
            else if getData?.photos?.photo?.count == 0 {
